...
export const ConfirmHealthOraclePlan = (): JSX.Element => {
  ...
  const handleSign = () => {
    // Do not submit here — navigate to the shared Review & Confirm page
    // so final confirmation and payload send happen centrally.
    const healthState = {
      executorName: (plan as any)?.executorName,
      executorEmail: (plan as any)?.executorEmail,
      executorWallet: (plan as any)?.executorWallet,
      acceptedDocs: (plan as any)?.acceptedDocs,
      jurisdiction: (plan as any)?.jurisdiction,
    };
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        ...
        <div className="space-y-6">
          ...
          <div className="border border-gray-800 rounded-2xl p-6 bg-gray-900/50">
            ...
            <button onClick={handleSign} disabled={isSubmitting} className={`w-full ${isSubmitting? 'bg-orange-400' : 'bg-orange-600 hover:bg-orange-700'} text-white font-semibold py-3 rounded-lg transition`}>
